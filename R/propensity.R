...
propensity <- function(DF,y,regression=FALSE){
   ...
   DF_propensity[unique(colnames(DF_propensity))] -> DF_propensity
   DF_propensity-> DF_propensity2##
   ...
   DF_propensity %>% select(-all_of(todelete)) ->DF_propensity
   #
   #
   ...
   plot(Perf,colorize = TRUE, main = "ROC propensity score",sub=paste("\nAUC = ", perftest@y.values[[1]]))
